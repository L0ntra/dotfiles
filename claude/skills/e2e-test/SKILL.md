...
Launch **three sub-agents simultaneously** using the Agent tool (`subagent_type: "Explore"` for research). Issue all three tool calls in a single message so they run in parallel.
...
2. Start the dev server **in the background** — use the Bash tool's `run_in_background: true` rather than a trailing `&`
...
Using the user journeys from Sub-agent 1 and findings from Sub-agent 3, use `TaskCreate` to add one task per user journey. Each task should include:
...
Mark each task `in_progress` with `TaskUpdate` when you start it and `completed` when its journey passes.

## Phase 4: User Journey Testing

For each task, execute the following.
